var SceneLoading = BaseScene.extend({
    ...
    ctor: function(){
        ...
        this.txtPercent.ignoreContentAdaptWithSize(true);
        this.curRunningScene = null;
    ...
    onEnterTransitionDidFinish: function(){
        this._super();
        this.schedule(this.startLoading,0);
    },
    setPercent:function(percent){
        this.loadingBar.setPercent(percent);
        this.txtPercent.setString(cc.formatStr("loading ...%s%...c.",percent));
    },
    setCurScene:function(scene){
        this.curRunningScene = scene;
    },
    setPreViewSceneId:function(sceneId){
        this.preViewSceneId = sceneId;
    },
    startLoading:function(){
        Log.debug("startLoading");
        this.unschedule(this.startLoading);
        if(this.curRunningScene){
            this.curRunningScene.clean && this.curRunningScene.clean();
            this.curRunningScene.release();
        }
        this.nextScene = sceneMgr.getScene(this.preViewSceneId);
        if(this.nextScene){
            this.nextScene.preEnter && this.nextScene.preEnter();
            this.nextScene.retain();
        }else{
            popups.showMessage("failed load scene:%s",Log.getKey(SCENE_IDs,this.preViewSceneId));
        }
    },
    /**
     *
     ...
     */
    onShowLoaded:function(percent){
        this.setPercent(percent);
        if(percent>= 100){
            this.onLoadedDone();
        }
    },
    onLoadedDone:function(){
...
//                console.log(">>" + data);
        ...
        var btnWritePerm = new cc.MenuItemFont("Write Perm", function(){
            sdkbox.PluginFacebook.requestPublishPermissions(["user_posts"]);
        }, this);
